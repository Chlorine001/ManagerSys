  ...
  <div>
    ...
    <div style="margin-bottom: 20px;padding:30px 20px; background-color: white;"
      ...
      <div style="display: flex;margin-bottom: 20px;">
        ...
        <el-input v-if="type == 1" :value="currentUser?.name || 'Admin'"></el-input>
        <div v-else>{{ currentUser?.name || 'Admin' }}</div>
      </div>
      <div style="display: flex;margin-bottom: 20px;">
...
import {getCurrentUser, getCurrentUserName} from '@/utils/user'
...
export default {
  ...
  mounted() {
    ...
    console.log('个人资料页面 - 当前用户:', this.currentUser)
  },
  methods: {
    getCurrentUser,
    getCurrentUserName,
    loginout() {
      ...
      if (confirms) {
        //token 过期也按如下方式处理清除信息，不建议直接 clear 全部本地缓存清除
